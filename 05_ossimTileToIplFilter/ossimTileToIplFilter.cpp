		...
		ossim_uint32 resLevel)
{
	...
	if (!imageSourceData.valid() || !theTile.valid())
		...
		IplImage *input = cvCreateImage(cvSize(tileRect.width(), tileRect.height()),IPL_DEPTH_8U,1);
		...
		IplImage *subImg = cvCreateImage(cvSize(tileWidth, tileHeight),IPL_DEPTH_8U,1);
...
void ossimTileToIplFilter::CopyTileToIplImage(T dummyVariable, ossimRefPtr<ossimImageData> inputTile, IplImage *output, ossimIrect neighborhoodRect)
{
	...
	{
		T* inBuf = static_cast<T*>(inputTile->getBuf(0));
		for (long y = 0; y < output->height; ++y)
		{
			for (long x = 0; x < output->width; ++x)
			{
				...
				if ((int)round(pixVal * scFactor) > 255)
					outputData[y * outputStep + x] = 255;
				else if ((int)round(pixVal * scFactor) < 0)
					outputData[y * outputStep + x] = 0;
				else
					outputData[y * outputStep + x] = (uchar)round(pixVal * scFactor);
